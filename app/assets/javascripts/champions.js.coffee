...
window.onload = -> 
  $('.level-slider').bind "slider:changed", (event, data) ->
    ...
    $('span.stat.attack-speed').html(baseAttackSpeed + (baseAttackSpeed * attackSpeedPerLvl * (data.value - 1)))

    baseArmor = parseFloat $('span.stat.armor').data('armor')
    armorPerLvl = parseFloat $('span.stat.armor-per-lvl').html()
    $('span.stat.armor').html(baseArmor + armorPerLvl * (data.value - 1))
    ...
    $('span.stat.magic-resist').html(baseMagicRes + magicResPerLvl * (data.value - 1))
